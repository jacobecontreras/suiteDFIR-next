...
/// dependencies in the crate table.
#[test]
fn the_embedded_licenses_are_the_generated_notices() {
    let notices = super::LICENSES;
    assert!(
        notices.starts_with("# Third-party notices\n\nGenerated by `cargo xtask notices`"),
        "{}",
        &notices[..notices.len().min(200)]
    );
    for needle in [
