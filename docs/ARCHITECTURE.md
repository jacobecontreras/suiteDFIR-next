...
1. **Never alter evidence.** The app never writes inside an input path. Inputs are opened read-only for hashing and inspection. The parsers read inputs and write only to the run folder. Runs whose output would land inside the input are refused (§6 step 1).
...
| D15 | **Passwords are never persisted.** They are redacted in `run.json` argv. stdin is null and there is no controlling terminal, so LEAPP can never block on a prompt. | argv is the only non-interactive channel (visible in `ps`); accepted and documented. |
...
| D21 | **iOS acquisition uses the libimobiledevice CLI tools** (`idevice_id`, `ideviceinfo`, `idevicepair`, `idevicebackup2`) as subprocesses, via the same `process` module as LEAPP (session/job, cancel escalation, 30 s grace for backups). | The proven, open toolset; the tools abort gracefully on SIGTERM and flush progress (IDEVICE-CLI.md). |
| D22 | **Tool binaries are built from pinned upstream source tarballs** by our own reproducible script/CI (X1), for macOS arm64/x64 and Windows x64. They are pinned by SHA-256 in `idevice-tools.json` and bundled as Tauri sidecars in release builds only. **Linux uses the distro's tools** (usbmuxd must be system-installed anyway). No binaries of unknown provenance, including the legacy repo's, are ever shipped. | Forensic defensibility and GPL source obligations. |
| D23 | **Enabling backup encryption is an explicit, recorded examiner action.** The password is never stored. The app offers to turn encryption off again afterwards. It never resets device settings. | Encryption changes device state; the record must show it. |
...
| `fsutil` | `write_json_atomic`, read-only marking, path-overlap checks (canonicalize for comparison only). | M0.3 (+C1) |
| `hashing` | `sha256_file` (M0.3). Progress/cancel variant and the report seal writer (C3). | M0.3, C3 |
...
| `acquire` | One acquisition end-to-end (§6b) via a callback: `acquisition.json` lifecycle and recovery, backup process, progress parsing, validation, seal, and restoring the encryption setting. | X3 |
...
- **Lifecycle:**
  - The quit guard handles `WindowEvent::CloseRequested` and `RunEvent::ExitRequested`. During a run it shows a native Rust-side dialog (`tauri_plugin_dialog`) asking "cancel run and quit?". On yes, it cancels, waits up to 30 s for finalize, then exits.
...
1. **Discover** (`devices_list`; the UI polls every 2 s while the Acquire screen is open):
   - `idevice_id -l`.
   - For each UDID: `ideviceinfo -u <udid> -s -x` for identity, then `idevicepair -u <udid> validate` → pair state (IDEVICE-CLI.md §4).
   - If paired: `WillEncrypt` and disk usage.
   - Tool-missing and usbmuxd-unavailable conditions become a device-list error state with platform-specific guidance, not an exception.
2. **Pair** (`device_pair`): `idevicepair pair`. The UI shows "Unlock the device and tap Trust" and retries on `awaiting_trust` or `locked`.
3. **Validate** (`acq_start`; failures create nothing):
   - no active job;
   - the device is present and paired;
   - the tools are present and, when bundled, their hashes match `idevice-tools.json`;
   - if `enable_encryption`: the password is given (≥ 4 chars, entered twice in the UI) and the device's `WillEncrypt` is false;
   - free disk space in the case folder ≥ the device's used data capacity (warn below; block below half of it).
4. **Prepare:**
   - Create `acquisitions/<acq_id>/`.
   - Write the initial `acquisition.json` (`status: running`) with a snapshot of the device identity.
   - Create the per-job temp dir.
5. **Enable encryption** (if requested): `idevicebackup2 -u <udid> encryption on <pw>`. Failure → `failed` (`encryption_enable_failed`). Record `enabled_by_examiner: true`.
6. **Back up:**
   - `idevicebackup2 -u <udid> backup --full <acq_dir>/backup`, with stdout/stderr to files.
   - Parse progress (`\]\s+(\d+)%`) → `progress` events; other lines → `log` events.
   - Cancel before exit: SIGTERM the group, then SIGKILL after 30 s (Unix); terminate the job (Windows).
7. **Restore the encryption setting** (if encryption was enabled in step 5 and `restore_encryption` is true, whatever the backup outcome): `encryption off <pw>`. Failure → warning `encryption_restore_failed` (the device keeps encryption on; the UI tells the examiner).
8. **Validate the backup** per IDEVICE-CLI.md §6 → status and reasons (CONTRACTS.md §13.3).
9. **Seal:** if `backup/` exists, write `backup.sha256` (same format and code as `report.sha256`).
10. **Finalize:**
    - Write the complete `acquisition.json` atomically and mark it read-only.
    - Emit `finished`.
    - Clear the active job.
    - The core keeps no password. If the examiner ticked "Parse with iLEAPP now", the UI (which already holds the password) pre-fills New run and clears the password once that run starts or the form closes (CONTRACTS.md §13.5).

Recovery on `case_open` mirrors runs: a `running` acquisition that is not this process's active job → `interrupted`.
...
- **Unix:**
  ...
  - Cancel: `killpg(pgid, SIGTERM)`, wait up to 10 s, then `killpg(pgid, SIGKILL)`.
...
<Case Name>/
  ...
  acquisitions/
    20260924-171200Z-ios-9c01de/
      acquisition.json       read-only once finalized
...
| `acq_start.case_path`, `acq_get`, `devices_*` | `case_path` must be a known case folder. Devices are addressed by a UDID matching `^[0-9A-Fa-f-]{24,40}$`. |
...
- **Secrets:**
  - Backup passwords (for parsing, and for enabling/disabling encryption) live only in memory for the duration of spawn. The acquisition→parse handoff is held by the UI, not the core (§6b step 10).
  ...
  - Passwords passed as argv are visible in the OS process list while the tool runs; this is accepted and documented.
...
- **iOS acquisition prerequisites:**
  ...
  - Windows: the Apple Mobile Device Service (Apple Devices app or iTunes). The app detects its absence and links to installation guidance.
