...
> **Not ready for use.** suiteDFIR is in development and has no releases yet. Parts of it haven't been tested with real devices or real evidence, so please don't rely on it for casework.

suiteDFIR is a desktop app for mobile forensics. It runs [iLEAPP](https://github.com/abrignoni/iLEAPP) and [aLEAPP](https://github.com/abrignoni/ALEAPP), the open-source iOS and Android parsers by Alexis Brignoni, and it can take an iOS backup from a phone connected over USB. Each case is a plain folder on disk, and every parser run and every backup gets a record of what was run, on what input, and what came out.
...
iOS backups aren't available on Windows arm64, because there's no build of the iOS tools for it. On Linux, the pinned iLEAPP and aLEAPP builds need glibc 2.43 or newer (Ubuntu 26.04, for example).
...
suiteDFIR-next was written entirely by AI. I used Claude Code, Anthropic's coding tool, to run a group of Claude agents. One agent planned the work and split it into tasks, others wrote the code, tests, documentation, CI and build scripts, and separate agents reviewed the pull requests. I set the goals, made the decisions the agents brought to me, and did the hands-on testing. From the first commit to the current state took about 29 hours, on September 24 and 25, 2026, and produced roughly 65,000 lines of code and tests.

Some of what you see on GitHub was posted by the agents from my account. The "Independent review" comments on pull requests came from AI reviewers, and the `gate/macos` and `gate/windows` commit statuses record test runs that agents did on my machines. Commits don't carry AI co-author tags, so this section is the disclosure for the whole history.

There are 547 Rust tests and 192 JavaScript tests. Where they involve iLEAPP, aLEAPP or the iOS tools, they mostly use stand-ins, which keeps them fast and repeatable. The real parsers run in CI on small sample inputs. Checks against real evidence and real devices come next, and that's where the work is now:
